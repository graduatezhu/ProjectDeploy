...
class ChargeInterfaceController extends CommonController{
	...
	public function stationList() {
        ...
        $data=$station->field('id,name,lat,lng')->select();//进行数据查询
    ...
    * @param int paymentMethod 电站收费模式 1 app 2电卡 3现金 0 全部
    * @param float lat 用户经度
    * @param float lng 用户纬度
    * @return JSON
    * @throws
    */
    ...
    public function searchStationList(){
        ...
        //检查并处理传来的数据
        if(isset($_POST['userid']) && isset($_POST['open']) && isset($_POST['order']) && isset($_POST['city']) && isset($_POST['chargeMethod']) && isset($_POST['parkingMethod']) && isset($_POST['paymentMethod']) && isset($_POST['lat']) && isset($_POST['lng'])){
        ...
        switch ($paymentMethod){
            case 1:
                $where['charge_app']=array('EQ',0);
                break;
            case 2:
                $where['charge_card']=array('EQ',0);
                break;
            case 3:
                $where['charge_cash']=array('EQ',0);
                break;
        }
        ...
        $station=M('charge_station');//实例化数据库模型
        $data=$station->field('id,name,phone,carrieroperator,AC_num,DC_num,charge_app,charge_cash,charge_card,lat,lng,charging_fee,parking_fee,score')->where($where)->select();
	...
	public function getChargeCollection(){
        ...
        $station_data=$station->field('id,name,address,score,parking_fee,lat,lng')->where("id IN({$storedata['charge_id']})")->select();
    ...
    public function searchCollection(){
        $return_data=[];//声明一个用来存放返回数据的数组
        if(isset($_POST['userid']) && isset($_POST['search'])){
            $userid=intval($_POST['userid']);
        ...
        $station=M('charge_station');
        $stationdata=$station->field('id,name,lat,lng')->where("name like '%{$search}%'")->select();;
        ...
        $collection=M('collection')->field('charge_id')->where("id={$userid}")->find();

        if(empty($collection)){
            $return_data['success']=true;
            $return_data['status']=1;
            $return_data['message']='命令执行成功';
            $return_data['info']=$stationdata;
            echo json_encode($return_data,JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);//输出组装好的数据
            exit;
        }else{
            $collection_station=explode(',',$collection['charge_id']);
            foreach($stationdata as $k => $v){
                in_array($v['id'],$collection_station)? $stationdata[$k]['collection']=0 : $stationdata[$k]['collection']=1 ;
            }
