...
use Think\Model;
class EZcStationModel extends Model{
	...
	public function zsearchs($val,$lat,$lng){
		...
		foreach ($result as $ke => $va) {
			...
			//距离km
			$result[$ke]['facemiles']=getDistance($lat,$lng,$va['lat'],$va['lng']);
			//空闲车辆数
			...
			$result[$ke]['freecar']=$this->pubsell($fieldd,$joinn,$map,'');
			//最大续航
			$fieldq='e_zc_cars.sn';
